...
def nws_latest_obs(lat: float, lon: float):
    ...
    NOTE: Some cities' "nearest" station occasionally returns null temperature.
    Fix: try multiple nearby stations and return the first one with a numeric temp.
    """
    points = nws_points(lat, lon)
    ...
    # Try several nearby stations (the first one isn't always usable).
    ...
    # If no station had a numeric temperature, still return tz + last timestamp we saw.
    return None, tz_name, last_obs_time
